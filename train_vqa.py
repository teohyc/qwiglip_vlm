...
from transformers import AutoTokenizer, AutoProcessor, AutoModel, Qwen2ForCausalLM, get_scheduler
...
#load dataset
coco_dataset = load_from_disk("coco_chat_dataset")
vqa_dataset = load_from_disk("vqa5k_chat_dataset")

#merge datasets
coco_subset = coco_dataset.select(range(1250)) #select 80:20 split to prevent forgetting VQA format and catastrophic forgetting of COCO data
#coco_subset = coco_subset.map(convert_coco) #unify format with vqa dataset

#cleaning
vqa_dataset = vqa_dataset.remove_columns(
    [col for col in vqa_dataset.column_names if col not in ["image_path", "messages"]]
)
coco_subset = coco_subset.remove_columns(
    [col for col in coco_subset.column_names if col not in ["image_path", "messages"]]
)

dataset = concatenate_datasets([coco_subset, vqa_dataset])
...
state_dict = torch.load("qwiglip_vlm.pt", map_location=DEVICE)
...
for epoch in range(EPOCHS):
    ...
    if val_loss < best_val_loss:
        ...
        torch.save(model.state_dict(), "qwiglip_vqa.pt")
